...
from Viajar.models import Viaje,Edicion,Comentario #aquí importo los del models.
...
def ediciones(request,viaje_id): #viaje_id es la clave primaria que te crea django por defecto.
	if request.method == "POST":
		...
		if form.is_valid():
			...
			comentario.viaje = get_object_or_404(Viaje, pk = viaje_id)
			comentario.save()
			return HttpResponseRedirect("/ediciones/"+viaje_id)#al meter el comentario que me redireccione en la página que estoy.
	else:
		form = ComentarioForm()
		comentarios = Comentario.objects.filter(viaje=viaje_id )
	...
	return render (request, 'Viajar/ediciones.html', {'ediciones': ediciones,'form':form,'comentarios':comentarios})	#con esto le digo que me lleve al html(me lo saca en la pagina)
